...
function enProceso(
  ...
  if (solicitud.estado === 'contratada') {
    // A partir de aquí manda el trabajo: la solicitud ya cumplió su papel.
    const vista = job ? vistaDelTrabajo(job, 'provider', { contraparte: cliente }) : null;
    const jobId = cotizacion?.jobId;
    if (!jobId) return null;
    return {
      ...base,
      etapa: 'trabajo',
      chip: vista?.etiqueta ?? 'Trabajo en curso',
      chipIcono: null,
      chipClase: job && requiereAccionDe(job, 'provider') ? CHIP_ACCION : CHIP_HECHO,
      detalle: vista?.titulo,
      siguiente: vista?.principal ? vista.detalle : undefined,
      cta: 'Continuar trabajo',
      destino: `/profesional/trabajos/${jobId}`,
      requiereAccion: job ? requiereAccionDe(job, 'provider') : false,
    };
  }
...
export function ProviderActivityScreen() {
  ...
  const jobs = useJobsDe(direccionDelProfesional(providerProfile));
  ...
  const vacio = proceso.length === 0 && oportunidades.length === 0;
  ...
  return <Screen header={<ScreenHeader title="Solicitudes" back={false} />}>
    <div className="px-4 py-6">
      ...
      {oportunidades.length > 0 && <section aria-labelledby="oportunidades" className={cn(proceso.length > 0 && 'mt-8')}>
