            ...
            return <li key={item.id} className="rounded-masi-card border border-masi-gray bg-white p-4 shadow-masi-sm">
              <div className="flex flex-wrap items-start justify-between gap-x-3 gap-y-1.5">
                <p className="flex min-w-0 items-center gap-1.5 text-sm font-semibold text-masi-blue">
                  ...
                  <span className="min-w-0 break-words">{solicitud?.servicio ?? 'Solicitud'}</span>
                </p>
                <span className={`flex max-w-full items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${estado.clase}`}>
                  <EstadoIcon size={13} aria-hidden="true" className="shrink-0" />
                  <span className="min-w-0 break-words">{estado.texto}</span>
                </span>
              </div>
              ...
              {solicitud?.estado === 'profesional_elegido' && solicitud.postulacionElegidaId === item.id && <Button
                className="mt-3"
                onClick={() => navigate(`/profesional/cotizacion/${solicitud.id}`)}
              >Enviar cotización final</Button>}

              {solicitud?.estado === 'cotizada' && solicitud.postulacionElegidaId === item.id && <p className="mt-3 text-sm text-masi-muted">
                Cotización enviada. Esperando la respuesta del cliente.
              </p>}
            </li>;
          })}
        </ul>}
