...
export function WelcomeScreen() {
  ...
  return <Screen footer={
    ...
    <div className="flex min-h-full flex-col justify-center bg-linear-135 from-masi-bg to-masi-blue-50 px-6 pt-[calc(2.5rem+var(--masi-safe-top))] pb-10">
      <MasiLogo variant="app" className="mx-auto w-36 max-w-[45%]" />
      <h1 className="mt-8 text-center text-[32px] leading-tight font-extrabold tracking-tight text-masi-navy">
        Tu hogar en<br />buenas manos
      </h1>
      <span className="mx-auto mt-5 block h-1 w-14 rounded-full bg-masi-orange" />
      <p className="mt-5 text-center text-base text-masi-text">
        Encuentra a quien sabe hacerlo y paga con tranquilidad: tu dinero queda protegido hasta que el trabajo esté listo.
      </p>
      <ul className="mt-8 space-y-3">
        {PROMISES.map(({ icon: Icon, text }) => <li key={text} className="flex items-center gap-3 rounded-masi-card border border-masi-gray bg-white px-4 py-3 shadow-masi-sm">
          <span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-masi-blue-50 text-masi-blue"><Icon size={20} aria-hidden="true" /></span>
