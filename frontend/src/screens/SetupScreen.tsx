...
export function SetupScreen() {
  ...
  return <Screen
    header={<ScreenHeader title="Cuéntanos un poco sobre ti" subtitle="Después crearás tu llave de acceso" />}
    footer={<ScreenFooter className="border-t border-masi-gray bg-white">
      <Button type="submit" form="perfil" disabled={!ready || busy}>
  ...
  >
    <form id="perfil" onSubmit={submit} className="space-y-4 px-4 py-6">
      ...
      {pending && <p className="rounded-masi-input bg-masi-cream p-3 text-sm text-masi-navy">
        Hay un registro pendiente en este navegador. Reintenta con el mismo nombre para terminarlo sin crear otra cuenta.
      </p>}
      {error && <p role="alert" className="rounded-masi-input bg-masi-blue-50 p-3 text-sm text-masi-error">{error}</p>}
      <p className="text-sm text-masi-muted">Tu celular te pedirá confirmar la llave de acceso una sola vez durante el registro.</p>
